...
-- Parser for single digit integer
digitP :: Parser Int
digitP = digitToInt <$> (charP '0' <|> charP '1' <|> charP '2' <|> charP '3' <|> charP '4' <|> charP '5' <|> charP '6' <|> charP '7' <|> charP '8' <|> charP '9')

-- Parser that takes account of '-' for negative Integers
intNegP :: Parser Int
intNegP = negate <$> (charP '-' *> intP)

-- Parser for any unicode letter
letterP :: Parser Char
letterP = Parser f
  where
    f (x : xs)
        | isLetter x = Just (x, xs)
        | otherwise = Nothing
    f [] = Nothing
